 ...
 * @param[in] net_params The network params.
 * @param[in] net_type The network type.
...
std::vector<std::string> psi(const std::vector<std::string>& input, std::size_t party_id, bool obtain_result,
        bool verbose, const petace::network::NetParams& net_params, petace::network::NetScheme net_type,
        petace::setops::PSIScheme psi_scheme);

/**
 * @brief PJC python api warpper.
 *
 * Both parties input their sets and get secert share of the intersection of their sets.
 *
...
std::vector<std::vector<std::uint64_t>> pjc(const std::vector<std::string>& keys,
        ...
        const petace::network::NetParams& net_params, petace::network::NetScheme net_type,
        petace::setops::PJCScheme pjc_scheme);
